...
`scripts/notify.py` 가 판단합니다. 이 여섯 가지 말고는 아무 말도 하지 않습니다 —
조용하면 정상이라는 뜻이고, 조용한데 정상이 아닌 경우를 없애는 것이 이 목록의 목적입니다.

| 알림 | 뜻 |
|---|---|
| `clears every check` | 셀 하나가 모든 검사를 통과했습니다. 이 장치 전체가 만들어내려던 것입니다 |
| `ORC stalled` | 30시간 넘게 사이클이 끝나지 않았습니다 |
| `ORC is idle` | 양쪽 다 돌았는데 이틀째 새 시행이 원장에 들어오지 않았습니다. 제안이 안 나오거나, 전부 기각되거나, 그리드가 소진된 것입니다 |
| `never reached the remote` | 로컬에만 커밋이 있습니다. 워커는 원격에서 수거하므로 그 질문은 영원히 평가되지 않습니다 |
| `BLOCKING finding` | high 등급 리뷰 발견이 미처리 상태입니다. 이게 있으면 사이클이 아예 돌지 않습니다 |
| `rejected a queued hypothesis` | 큐 파일이 스키마·상한을 어겨 `rejected/` 로 갔습니다 |

`ORC is idle` 와 `never reached the remote` 가 특히 중요합니다. 나머지는 뭔가
실패하지만, 이 둘은 **전부 성공한 것처럼 보이는 채로** 연구가 멈춘 상태입니다.

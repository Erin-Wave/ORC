...
# oracle/ added 2026-09-05, and the refusal that made it necessary is the check
# working twice over: a new top-level directory is where a tool writing into the
# tree lands, and this one holds a file a tool DID write -- ref_signal.py, the
# second implementation of run_signals, generated by codex.
#
# It is a separate tree on purpose. Nothing in oracle/ is imported by the
# research path, none of it is in CODE_HASH_ROOTS, and it can never produce a
# ledger row; that is what makes it safe to regenerate continuously. Putting it
# under tests/ would have made it look like a test, and it is not one -- it is
# a second opinion that happens to disagree.
EXPECTED_DIRS = ("orc/", "scripts/", "tests/", "configs/", "reports/",
                 "ledger/", "docs/", ".github/", "oracle/")
